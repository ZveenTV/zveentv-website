(function(){
  ...
  function watch(data){let queued=false;const obs=new MutationObserver(()=>{if(queued)return;queued=true;queueMicrotask(()=>{queued=false;apply(data)})});obs.observe(document.body,{subtree:true,childList:true,characterData:true})}
  window.ZVEEN_TEXT_CMS={apply,watch,keyFor,nodes,pageName};
})();
